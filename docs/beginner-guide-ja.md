...
ここでの `Γ` は「式を読む前」の入力 environment (入力環境)、`Γ'` は「式を読んだ後」の出力 environment (出力環境) である。
記号 `⊣`（Unicode name は `LEFT TACK`、LaTeX では `\dashv`）は、その 2 つを区切る separator (区切り記号) であり、
「左側の式を処理した結果、右側の environment が得られる」と読めばよい。
